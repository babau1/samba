...
if __name__ == "__main__":
    ...
    for dataset_name, view_name in zip(dataset_names, view_names):
        ...
        if isinstance(view_name, list):
            res = pd.read_csv("/home/baptiste/Documents/Gitwork/neighborhood_classifier/demo/Performance /results/metagenome/debug_started_2022_11_25-11_47_00_th/metagenome-mean_on_10_iter-balanced_accuracy_p.csv",
            ...
            for v_name in view_name:
                full_res = full_res.append(dict(dict((alg_dict[plif.split("-")[0]],
                                                      "{} $\pm$ {}".format(round(res[plif]["Test"], 2),
                                                                         round(res[plif]["Test STD"], 2)))
                ...
                test_res_df = test_res_df.append(dict(dict((" ".join(col.split("-")[0].split("_")).title(), "{} \\scriptsize ({}) \\normalsize ".format(round(scores[col]['Test'], 2), n_feat[col.split("-")[0]])) for col in scores.columns), **{"Dataset":v_name + " \\hfill ({})".format(view_sizes[v_name])}), ignore_index=True)
        else:
    ...
    latex = test_res_df[["Dataset", "Adaboost", "Decision Tree", 'Samba', 'Random Forest', 'Svm Rbf', "Knn"]].to_latex(index=False, escape=False, column_format="|l|c|c|c|c|c|",)
    latex = latex.replace("\\toprule\n", "")
    latex = latex.replace("\\midrule\n", "")
    latex = latex.replace("\\bottomrule\n", "")
    latex = latex.replace("\n", " \\hline\n")
