...
class LassoClf(Lasso):
    ...
    def predict(self, X):
        preds = super(LassoClf, self).predict(X)
        preds = np.array([pred > 0.5 for pred in preds]).astype(int)
        return preds
...
def plot_contour_fig(X, y, clf=AdaBoostClassifier(), contour=True,
...
def plot_contour_img(classifiers, X_train, y_train, X_test, y_test, n_steps, dataset,
    ...
    for classifier in classifiers:
        fig = plot_contour_fig(X_train, y_train, clf=classifier,
